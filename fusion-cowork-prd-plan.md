# Fusion-Cowork 整改方案与实施计划

> 生成日期: 2026-07-28 | 基于 claude-cowork-insight.md 对标结论 | 仅修改 fusion-cowork + fusion-studio

---

## 一、整改原则

1. **只改两个项目**: fusion-cowork (Python 后端) + fusion-studio (SwiftUI GUI)
...
**目标**: fusion-cowork 可作为 MCP server 被 Claude Desktop/Code 调用

**fusion-cowork 改动**:

```
fusion_cowork/server/
...
**目标**: fusion-studio DeskView 可调用 fusion-cowork 工作流引擎

**fusion-cowork 改动** — 新增 JSON-RPC 服务端:

```python
# fusion_cowork/server/desk_rpc.py (新增)
class DeskRPCServer:
    """JSON-RPC 2.0 服务端 — 供 fusion-studio 通过 UDS 调用。"""

    # 监听 /tmp/fusion-cowork.sock
    # 方法:
...
// 1. 注入 IPCClient，连接 /tmp/fusion-cowork.sock
...
- 新增命名空间 `desk.*`
- socket 路径: `/tmp/fusion-cowork.sock` (与 `/tmp/fusion-studio.sock` 并行)

**验收标准**:
- [ ] fusion-studio DeskView 显示 fusion-cowork 的真实模板列表
- [ ] 点击"运行"→ fusion-cowork 执行工作流 → 结果回传到 UI
...
```python
# fusion_cowork/orchestrator/orchestrator.py 重构
...
```python
# fusion_cowork/orchestrator/executors.py (新增)
class NodeExecutor:
...
```python
# fusion_cowork/orchestrator/comm.py (新增)
...
```python
# fusion_cowork/engine/permission.py (新增)
...
```python
# fusion_cowork/engine/hooks.py (新增)
...
```python
# fusion_cowork/engine/session.py (新增)
@dataclass
class Session:
    session_id: str
    workflow: Workflow
...
class SessionStore:
    """会话存储 — SQLite 持久化。"""
    db_path: str = "~/.fusion-cowork/sessions.db"
...
**CLI 集成**:
```bash
fusion-cowork workflow resume <session-id>
fusion-cowork workflow list-sessions
fusion-cowork workflow fork <session-id>
```
...
```python
# fusion_cowork/engine/events.py (新增)
class WorkflowEvent:
...
```python
# fusion_cowork/plugins/ (新增目录)
# fusion_cowork/plugins/loader.py
class PluginLoader:
    """插件加载器 — 从目录/zip 动态加载。"""
...
```python
# fusion_cowork/skills/ (新增目录)
# fusion_cowork/skills/registry.py
class SkillRegistry:
    """技能注册表 — 快捷命令映射到工作流/节点。"""
...
**对标全量**: Claude Cowork 有 25+ CDP 子工具，Fusion-Cowork 需覆盖核心子集
...
```python
# fusion_cowork/nodes/browser/cdp_client.py (新增)
class CDPClient:
...
```python
# fusion_cowork/nodes/macos/input_nodes.py (新增)
@register_node
...
**目标**: 外部可连接运行中的 fusion-cowork 会话

**fusion-cowork 改动**:

```python
# fusion_cowork/server/remote.py (新增)
...
```python
# fusion_cowork/engine/schema.py (新增)
class OutputSchema:
...
```python
# fusion_cowork/orchestrator/orchestrator.py (增强)
class AgentOrchestrator:
...
# fusion_cowork/orchestrator/executors.py (增强)
class CoordinatorExecutor:
    """协调执行器 — 调度子任务给其他 agent。"""
    async def __call__(self, input_data):
        # 将大任务分解为子任务，提交给匹配的 executor agent

# fusion_cowork/orchestrator/agent_runtime.py (新增)
class AgentRuntime:
...
```python
# fusion_cowork/engine/workflow.py (增强)
...
# fusion_cowork/engine/permission.py (增强)
class PermissionManager:
    hook_manager: Optional[HookManager] = None
...
# fusion_cowork/engine/hooks.py (增强)
class HookEvent(Enum):
...
```python
# fusion_cowork/sdk/__init__.py (新增)
...
# fusion_cowork/sdk/headless.py (新增)
...
# fusion_cowork/server/mcp_http.py (增强)
...
| 阶段 | 改动 | 文件 |
|------|------|------|
| M1 | DeskView 接入 fusion-cowork IPC | DeskView.swift 重构 |
...
| 目标项目 | Issue 标题 | 内容摘要 |
|----------|-----------|----------|
| fusion-mlx | 请求 vision/multimodal API 端点 | fusion-cowork 需要图像理解能力，目前 chat 接口不原生支持图片 |
...
| M | 验收项 | 验收方式 |
|---|--------|----------|
| M1 | MCP stdio 注册到 Claude Code | `claude mcp add fusion-cowork -- fusion-cowork mcp serve --transport stdio` 成功 |
