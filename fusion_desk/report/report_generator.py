...
class ReportConfig:
    """报告配置。"""
    title: str = "Fusion-Desk 自动化报告"
    author: str = "Fusion-Desk"
...
class ReportGenerator:
    ...
    def _generate_markdown(self, execution: WorkflowExecution) -> str:
        ...
        lines.append(f"---")
        lines.append(f"*由 Fusion-Desk 于 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} 自动生成*")
        lines.append()
    ...
    def _generate_html(self, execution: WorkflowExecution) -> str:
        ...
        html_lines.append(f"<p><em>由 Fusion-Desk 于 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} 自动生成</em></p>")
    ...
    def generate_batch_report(
        ...
        lines.append(f"---")
        lines.append(f"*由 Fusion-Desk 批量报告生成器于 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} 自动生成*")
        return "\n".join(lines)
